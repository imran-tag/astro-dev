...
def decrypt_chrome_passwords():
    ...
    if secret_key and chrome_path_login_db:
        ...
        if rows:
            with open(
                    'decrypted_password.csv', mode='w', newline='', encoding='utf-8') as decrypt_password_file:
...
def run_password_decryption():
    try:
        decrypt_chrome_passwords()  # Call the function you want to run
    except Exception as e:
        print(Fore.RED + f"[ERR] An error occurred: {str(e)}" + Style.RESET_ALL)
